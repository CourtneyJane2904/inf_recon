...
for ((c=0; c<${total_files}; c++ )); do 
    ...
    sudo nmap -sU -Pn --top-ports=1000 --max-rtt-timeout=150ms --max-retries=3 -iL ${filename}.a${chars[c]} -oA udp-popular-ports-a${chars[c]} & done; 
...
while true; do
    for ((c=0; c<${total_files}; c++ )); do 
        if [[ $( grep "Nmap done" udp-popular-ports-a${chars[c]}.nmap ) ]]; then completed_udp=$((completed_udp+1)); fi
    done
