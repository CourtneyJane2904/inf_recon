...
for ((c=0; c<${total_files}; c++ )); do 
	...
	nmap -sS --max-rtt-timeout=150ms --max-retries=3 -T4 -Pn -p- -iL "${filename}".a${chars[c]} -oA tcp-all-ports-a${chars[c]} & done; 
...
while true; do
    for ((ch=0; ch<${total_files}; ch++ )); do 
        if [[ $( grep "Nmap done" tcp-all-ports-a${chars[ch]}.nmap ) ]]; then completed_tcp=$((completed_tcp+1)); fi
    done
